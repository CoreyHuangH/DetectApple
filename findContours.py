...
def findContours(imgTresh: np.ndarray, original_img: np.ndarray) -> np.ndarray:
    contours, hierarchy = cv2.findContours(imgTresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    for cnt in contours:
        ...
        if area > 1200:
            ...
            cv2.putText(imgContours, "Apple", (x + w // 2 - 10, y + h // 2 - 10), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                        (0, 255, 0), 1)
    return imgContours
